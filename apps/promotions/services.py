...
def get_valid_coupon(*, code: str, tenant=None) -> Coupon:
    normalized_code = code.strip().upper()
    queryset = Coupon.objects.prefetch_related("products", "categories")
    if tenant is not None:
        queryset = queryset.filter(tenant=tenant)
    try:
        coupon = queryset.get(code=normalized_code)
...
def validate_coupon_for_order(*, coupon: Coupon, order) -> None:
    ...
    if coupon.products.exists() or coupon.categories.exists():
        ...
        order_category_ids = set(order.items.values_list("product__category_id", flat=True))
        ...
        if not (order_product_ids & coupon_product_ids) and not (order_category_ids & coupon_category_ids):
...
def apply_coupon_to_order(*, order, code: str) -> dict:
    coupon = get_valid_coupon(code=code, tenant=order.tenant)
    ...
    return {"coupon": coupon, "discount": discount, "final_amount": final_amount}
